# Automatic parcel tracking from e-mail (IMAP → Poste Italiane)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Poste Italiane tracking code, and register it with `poste_italiane.track_parcel` — fully automatic, no extra custom component required.

Poste Italiane is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `poste_italiane.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ poste_italiane.track_parcel
                                     │
...
- This integration, with the `poste_italiane.track_parcel` action available (field `tracking_code`).
...
The confirmed formats have no safe shared prefix, so the regex matches a
6–40-character alphanumeric candidate:

```
(?<![0-9A-Za-z])[0-9A-Za-z]{6,40}(?![0-9A-Za-z])
```

The integration accepts `^[A-Za-z0-9]{6,40}$` after trimming surrounding whitespace. This broad matcher can also find order references, so restrict the IMAP sender or folder to known Poste Italiane messages and use the AI fallback when needed.
...
1. **Jinja eats backslashes in string literals.** A template stored as `regex_findall('\bCODE…')` silently becomes a **backspace character** (`\b` is a string escape), so the regex never matches — no error anywhere. That's why the pattern is backslash-free: `(?<![0-9A-Za-z])` lookarounds instead of `\b`, `[0-9]` instead of `\d`.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@poste_italianeshipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: CW000000000IT","initial":true,"folder":"INBOX","username":"test"}'
```

Then `poste_italiane.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
