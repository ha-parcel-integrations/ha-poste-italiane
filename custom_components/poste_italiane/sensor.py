"""Sensor platform for the Poste Italiane parcel tracker integration."""
...
from . import PosteItalianeConfigEntry
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: PosteItalianeConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Poste Italiane sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        PosteItalianeIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
        PosteItalianeAwaitingPickupSensor(coordinator, entry),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            PosteItalianeParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(PosteItalianeNextDeliverySensor(coordinator, entry))
    entities.append(PosteItalianeDeliveredParcelsSensor(coordinator, entry))
    entities.append(PosteItalianeLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class PosteItalianeAwaitingPickupSensor(
    CoordinatorEntity[PosteItalianeCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: PosteItalianeCoordinator, entry: ConfigEntry
    ) -> None:
...
class PosteItalianeIncomingParcelsSensor(
    CoordinatorEntity[PosteItalianeCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                PosteItalianeParcelSensor(self.coordinator, self._entry, barcode)
...
class PosteItalianeParcelSensor(CoordinatorEntity[PosteItalianeCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Poste Italiane parcel."""
    ...
    def __init__(
        self, coordinator: PosteItalianeCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class PosteItalianeNextDeliverySensor(
    CoordinatorEntity[PosteItalianeCoordinator], SensorEntity
):
...
class PosteItalianeDeliveredParcelsSensor(
    CoordinatorEntity[PosteItalianeCoordinator], SensorEntity
):
    """Recently delivered tracked Poste Italiane parcels."""
...
class PosteItalianeLastUpdateSensor(
    CoordinatorEntity[PosteItalianeCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Poste Italiane was last polled successfully."""
