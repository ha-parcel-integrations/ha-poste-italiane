"""Coordinator for the Poste Italiane parcel tracker integration.
...
from .api import PosteItalianeApiClient, PosteItalianeApiError
from .const import (
...
class PosteItalianeCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        errors = 0
        error_summaries: set[str] = set()
        retry_afters: list[float] = []
        saw_429 = False
        for code, result in zip(codes_to_fetch, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (PosteItalianeApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if isinstance(result, PosteItalianeApiError) and result.status_code:
                    error_summaries.add(f"HTTP {result.status_code}")
                else:
                    error_summaries.add(type(result).__name__)
                if (
                    isinstance(result, PosteItalianeApiError)
            ...
            # its key.
            result.setdefault("idTracciatura", code)
        ...
        if errors:
            _LOGGER.warning(
                "Poste Italiane refresh had %d failed parcel request(s): %s",
                errors,
                ", ".join(sorted(error_summaries)),
            )
        ...
        if saw_429:
            ...
            raise UpdateFailed(
                "Poste Italiane rate-limited (429)", retry_after=retry_after
            )
        self._consecutive_429 = 0

        if codes_to_fetch and errors == len(codes_to_fetch) and not raws_by_code:
            raise UpdateFailed("Poste Italiane unreachable for all tracked parcels")
