"""Services for the Poste Italiane parcel tracker integration.

`poste_italiane.track_parcel` / `poste_italiane.untrack_parcel` let you add or remove a
...
def _resolve_entry(hass: HomeAssistant):
    """Return the single Poste Italiane hub, or raise when it is not set up."""
    entries = hass.config_entries.async_entries(DOMAIN)
    if not entries:
        raise ServiceValidationError("Poste Italiane is not set up")
    return entries[0]


def async_setup_services(hass: HomeAssistant) -> None:
    """Register the Poste Italiane services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid Poste Italiane tracking code"
            )
...
def async_unload_services(hass: HomeAssistant) -> None:
    """Remove the Poste Italiane services (single-entry integration, so on unload)."""
