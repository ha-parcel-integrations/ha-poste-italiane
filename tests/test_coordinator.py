"""Tests for the Poste Italiane coordinator: fetching, caching and events.
...
from custom_components.poste_italiane.api import PosteItalianeApiError
...
from custom_components.poste_italiane.coordinator import PosteItalianeCoordinator

from .payloads import ACTIVE_CODE, DELIVERED_CODE, active_sample, delivered_sample

OTHER_CODE = "LX000000001JP"
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    ...
    sample["listaMovimenti"] = sample["listaMovimenti"][:-1]
    return sample
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = PosteItalianeApiError("HTTP 500")
...
async def test_no_delivery_time_events_without_a_confirmed_eta(hass):
    entry = _entry_with([{CONF_TRACKING_CODE: ACTIVE_CODE}])
    entry.add_to_hass(hass)
    client = AsyncMock()
    coordinator = PosteItalianeCoordinator(hass, client, entry)
