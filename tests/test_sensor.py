"""Tests for Poste Italiane sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.poste_italiane.const import ParcelStatus
from custom_components.poste_italiane.sensor import (
    PosteItalianeAwaitingPickupSensor,
    PosteItalianeDeliveredParcelsSensor,
    PosteItalianeIncomingParcelsSensor,
    PosteItalianeLastUpdateSensor,
    PosteItalianeNextDeliverySensor,
    PosteItalianeParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = PosteItalianeIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_awaiting_pickup_zero_when_no_parcels():
    sensor = PosteItalianeAwaitingPickupSensor(_coordinator([]), _entry())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = PosteItalianeParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = PosteItalianeParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = PosteItalianeNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = PosteItalianeNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = PosteItalianeDeliveredParcelsSensor(coordinator, _entry())
    assert sensor.native_value == 1
...
def test_last_update_sensor():
    ...
    sensor = PosteItalianeLastUpdateSensor(coordinator, _entry())
