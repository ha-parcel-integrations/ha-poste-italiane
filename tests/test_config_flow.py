"""Tests for the Poste Italiane config and options flow."""

from pytest_homeassistant_custom_component.common import MockConfigEntry

from custom_components.poste_italiane.config_flow import (
...
def test_normalize_tracking_code_trims_without_changing_case():
    assert normalize_tracking_code("  Lx000000000jP  ") == "Lx000000000jP"
...
async def test_user_flow_creates_hub_without_input(hass):
    ...
    assert result["title"] == "Poste Italiane"
...
async def test_options_add_parcel(hass):
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"tracking_codes": ["LX000000000JP"]}
    )
    assert result["type"] == "create_entry"
    assert result["data"][CONF_PARCELS] == [{CONF_TRACKING_CODE: "LX000000000JP"}]


async def test_options_trims_code_without_rewriting_it(hass):
    entry = _hub([])
    entry.add_to_hass(hass)
    result = await _open_options_step(hass, entry, "parcels")
    result = await hass.config_entries.options.async_configure(
...
async def test_options_de_duplicates_tracking_codes(hass):
    ...
    result = await _open_options_step(hass, entry, "parcels")
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {"tracking_codes": ["LX000000000JP", "LX000000000JP"]}
    )
    assert result["data"][CONF_PARCELS] == [{CONF_TRACKING_CODE: "LX000000000JP"}]
